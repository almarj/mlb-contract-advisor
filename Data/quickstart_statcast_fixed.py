"""
QUICK START - Statcast Data Collection (FIXED VERSION)
...
try:
    ...
    print(f"\n✗ Error: {e}")
    print("Make sure pybaseball is installed: pip3 install pybaseball")
    exit()
...
try:
    ...
    print("\nFirst 3 players:")
    print(batters_2024[['last_name, first_name', 'avg_hit_speed', 'brl_percent']].head(3))
    ...
    if not judge_stats.empty:
        ...
        print(f"Barrel Rate: {judge_stats['brl_percent'].values[0]:.1f}%")
        print(f"95+ mph Exit Velo %: {judge_stats['ev95percent'].values[0]:.1f}%")
        print(f"Sweet Spot %: {judge_stats['anglesweetspotpercent'].values[0]:.1f}%")
...
print("  • brl_percent (barrel %)")
print("  • ev95percent (hard hit %)")
print("  • max_hit_speed (peak power)")
print("  • anglesweetspotpercent (optimal launch angle %)")
