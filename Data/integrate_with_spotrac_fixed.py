"""
MLB Contract Advisor - Data Integration Script (HANDLES MISSING VALUES)
Merge Spotrac contracts + FanGraphs stats + Statcast metrics

This version handles missing/incomplete contract data gracefully.
"""
...
try:
    ...
    # Show how many have missing values
    print(f"\nData quality check:")
    print(f"  Missing AAV: {contracts['AAV'].isna().sum()}")
    print(f"  Missing length: {contracts['length'].isna().sum()}")
    print(f"  Missing year_signed: {contracts['year_signed'].isna().sum()}")
    print(f"  Missing age: {contracts['age_at_signing'].isna().sum()}")
    
    # Filter out contracts with missing critical data
    print(f"\nFiltering out incomplete contracts...")
    ...
    contracts_after = len(contracts)
    removed = contracts_before - contracts_after
    
    print(f"  Removed {removed} incomplete contracts")
    print(f"  Remaining: {contracts_after} contracts")
    
    # Convert to integers now that NaN values are removed
    ...
    print(f"  Positions: {sorted(contracts['position'].unique())}")
    print(f"  AAV range: ${contracts['AAV'].min():,.0f} - ${contracts['AAV'].max():,.0f}")
